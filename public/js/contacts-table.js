 ...
 * 
 * Provides advanced search and filtering capabilities with property-based queries.
...
class SearchFilterBar {
   ...
   * Requirements: 4.2, 4.3, 4.4, 4.5, 8.4
   * 
   * Supported filter syntax:
   * - tag:work
   * - group:family
   * - source:google
   * - circle:inner (inner, close, active, casual, acquaintance)
   * - location:NYC
   */
  parseQuery(query) {
    const filters = {};
  ...
  applyFilters(data, text, filters) {
    return data.filter(contact => {
      ...
      // Tag filter (Requirement 4.2)
      if (filters.tag && filters.tag.length > 0) {
        const contactTags = contact.tags ? contact.tags.map(t => t.text.toLowerCase()) : [];
  ...
  /**
   * Load autocomplete data (tags and groups)
  ...
  getAutocompleteSuggestions(query) {
    ...
    // Suggest filter syntax if user hasn't completed it
    if (lastWord.includes(':')) {
      ...
      switch (filterType.toLowerCase()) {
        ...
        case 'source':
          // Suggest source values
          const sources = ['google', 'manual'];
          ...
          circles.forEach(circle => {
            if (!filterValue || circle.includes(filterValue.toLowerCase())) {
              suggestions.push({
                ...
                description: 'Filter by Dunbar circle'
              });
            }
          });
          break;
      ...
      // Suggest filter types if no colon yet
      const filterTypes = [
        { type: 'tag', description: 'Filter by tag' },
        { type: 'group', description: 'Filter by group' },
        { type: 'source', description: 'Filter by source (google, manual)' },
        { type: 'circle', description: 'Filter by relationship circle' },
        { type: 'location', description: 'Filter by location' }
      ];
      
      filterTypes.forEach(({ type, description }) => {
        if (type.startsWith(lastWord.toLowerCase())) {
          suggestions.push({
            type: 'filter-type',
            value: type,
            display: `${type}:`,
  ...
  selectSuggestion(suggestion) {
    ...
    // Add a space after the suggestion for easy continuation
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { ContactsTable, InlineEditCell, AZScrollbar, SearchFilterBar };
}
