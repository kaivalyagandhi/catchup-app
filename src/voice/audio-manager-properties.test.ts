...
/**
 * Testable AudioManager for property-based testing
 * 
 * This class extracts the core audio level detection logic from the browser-based
 * AudioManager to make it testable in a Node.js environment.
 */
class TestableAudioManager {
  private config: {
    silenceThresholdDb: number;
    lowLevelThresholdDb: number;
  ...
  private warnings: {
    silence: Array<{ db: number; timestamp: number }>;
    lowLevel: Array<{ db: number; timestamp: number }>;
    clipping: Array<{ db: number; timestamp: number }>;
  };
  
  constructor(config: Partial<typeof TestableAudioManager.prototype.config> = {}) {
    this.config = {
   ...
   */
  checkAudioLevel(db: number, timestamp: number = Date.now()): void {
...
describe('AudioManager Property-Based Tests', () => {
  let manager: TestableAudioManager;
  ...
  describe('Property 4: Audio level warning accuracy', () => {
    it('should trigger low-level warning for levels < -40dB and > -50dB', () => {
      fc.assert(
        fc.property(
          fc.double({ min: -49.99, max: -40.01, noNaN: true }), // Between silence and low threshold
          (level) => {
            manager.reset();
    ...
    it('should trigger clipping warning for levels >= 0dB', () => {
      fc.assert(
        fc.property(
          fc.double({ min: 0, max: 10, noNaN: true }), // At or above clipping threshold
          (level) => {
            manager.reset();
            manager.checkAudioLevel(level);
    ...
    it('should NOT trigger warnings for normal levels (-40dB to 0dB)', () => {
      fc.assert(
        fc.property(
          fc.double({ min: -39.99, max: -0.01, noNaN: true }), // Normal range
          (level) => {
            manager.reset();
            manager.checkAudioLevel(level);
  ...
  describe('Property 3: Silence detection threshold', () => {
    it('should detect silence after 3 seconds of levels below threshold', () => {
      fc.assert(
        fc.property(
          fc.double({ min: -100, max: -50.01, noNaN: true }), // Below silence threshold
    ...
    it('should NOT detect silence if levels are above threshold', () => {
      fc.assert(
        fc.property(
          fc.double({ min: -49.99, max: 0, noNaN: true }), // Above silence threshold
          fc.integer({ min: 3000, max: 10000 }), // Duration >= 3 seconds
          (level, duration) => {
            manager.reset();
