...
import { PostgresWeeklyCatchupService, type ReviewActionData } from '../../contacts/weekly-catchup-service';
 ...
 * }
 */
router.post('/:sessionId/review', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId!;
    const { sessionId } = req.params;
 ...
 * Requirements: 7.2
 */
router.get('/:sessionId/progress', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId!;
    const { sessionId } = req.params;
 ...
 * Requirements: 7.3
 */
router.post('/:sessionId/complete', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId!;
    const { sessionId } = req.params;
