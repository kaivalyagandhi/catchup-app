...
const checkTestDataEnabled = (_req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  if (enabled === 'false') {
    res.status(403).json({ 
      ...
      message: 'Test data endpoints are disabled in this environment. Set ENABLE_TEST_DATA_ENDPOINTS=true to enable them.'
    });
    return;
  }
 ...
 */
router.post('/seed', checkTestDataEnabled, authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.userId) {
 ...
 */
router.post('/generate-suggestions', checkTestDataEnabled, authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ error: 'Not authenticated' });
      return;
 ...
 */
router.post('/clear', checkTestDataEnabled, authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.userId) {
      res.status(401).json({ error: 'Not authenticated' });
      return;
