 ...
 */
router.get('/quick-start-suggestions', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
      ...
      try {
        ...
        if (suggestion.confidence >= 60) {
          ...
          const metadataFactor = suggestion.factors.find(
            (f) => f.type === 'communication_frequency' && f.weight === 0.30
          );
...
function buildReasons(
  ...
  if (calendarEventCount > 0) {
    if (calendarEventCount >= 20) {
      ...
      reasons.push(`${calendarEventCount} shared calendar event${calendarEventCount > 1 ? 's' : ''}`);
    }
  }
